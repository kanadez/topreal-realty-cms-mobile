...
function Synonim(object_for){ // object_for - объект, к кто. подключаем систему. Например, property
    ...
    this.show = function(){ // формирует и показывает загруженный по запросу список синонимов
        ...
        for (var i = 0; i < this.list.length; i++){
            ...
            if ($('.autocomplete-container #synonim_'+this.list[i].id).length == 0){
                $('.autocomplete-container').append('<div class="synonim-item" id="synonim_'+this.list[i].id+'" onclick="synonim.insert('+i+')"><span class="synonim-icon synonim-icon-marker"></span><span class="synonim-item-query"><span class="synonim-matched">'+this.list[i].text+'</span></span></div>');
            }    
                ...
                case "search":
                    if (this.current_input.attr("geotype") === "route"){
                        if (this.list[key].place_id == null){
                            ...
                            search.geoloc.street = this.list[key].id;
                            $('#route_input').val(this.list[key].text);
